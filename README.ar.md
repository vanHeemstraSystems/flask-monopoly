...
1) Enter `flask_app`دليل:`$ cd flask_app`2) تشغيل`$ pip install -r requirements.txt`3) انسخ app/sample.env إلى app/.env:`$ cp app/sample.env app/.env`4) التحديث`app/.env`مع إعداد التكوين الخاص بك.
...
**نصيحة**: لرؤية كل شيء_مخصص_ commands, from within `flask_app`تشغيل الدليل`$ flask commands --help`من محطة.
