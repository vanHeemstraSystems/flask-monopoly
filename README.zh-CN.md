...
5）如果不存在，则在内部创建一个虚拟环境`flask_app`目录：`$ python3 -m venv .venv`6）启动虚拟环境，输入：`$ . .venv/bin/activate`
7) Run `(.venv) $ ./setup.sh`（macOS 或 Linux）或`(.venv) setup.bat`（视窗）
...
11）根据提示打开Web界面
12) 使用`CTRL+c` to exit the web server.
13) Alternatively run the flask command line interface: `(.venv) $ flask shell`14）执行任何flask命令：>>>
15) 使用`exit()`退出命令行界面。
