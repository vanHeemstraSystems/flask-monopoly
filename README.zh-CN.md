...
8) 如果出现找不到模块 Flask-Bcrypt 的错误，请运行`(.venv) $ pip install Flask-Bcrypt`
9) Create the database: `(.venv) $ flask commands create_db`10）运行烧瓶应用程序：`(.venv) $ flask run`（或者`$ python main.py`）
11）根据提示打开Web界面
