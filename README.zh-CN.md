...
1) 输入`flask_app`目录：`$ cd flask_app`2) 跑步`$ pip install -r requirements.txt`
...
8) 如果出现找不到模块 Flask-Bcrypt 的错误，请运行`(.venv) $ pip install Flask-Bcrypt`8）运行烧瓶应用程序：`(.venv) $ flask run`9）根据提示打开Web界面
