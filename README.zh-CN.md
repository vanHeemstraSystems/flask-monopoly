...
1) 输入`flask_app`目录：`$ cd flask_app`2) 跑步`$ pip install -r requirements.txt`3）将app/sample.env复制到app/.env：`$ cp app/sample.env app/.env`
...
**警告**: 请务必使用`pyScss`版本1.3.7（**不是** 1.4.0) as it would re-introduce a bug ("global flags not at the start of the expression at position 1"). **更新**： 使用[库萨斯](https://sass.github.io/libsass-python/)而不是`pyScss`.
