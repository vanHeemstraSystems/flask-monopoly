...
8) 如果出現找不到模組 Flask-Bcrypt 的錯誤，請執行`(.venv) $ pip install Flask-Bcrypt`9）建立資料庫：`(.venv) $ flask commands create_db`
10) Run the flask app: `(.venv) $ flask run`（或者`$ python main.py`）
11）根據提示開啟Web介面
