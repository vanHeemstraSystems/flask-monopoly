...
1) 輸入`flask_app`目錄：`$ cd flask_app`2) 跑步`pip install -r requirements.txt`3）如果不存在，則在內部建立一個虛擬環境`flask_app`目錄：`python3 -m venv .venv`4）啟動虛擬環境，輸入：`. .venv/bin/activate`
